...
const FormApBp = ({popup}) => {
    const data = [
        {
            ...
            'date': '2023-02-01',
            'reason': 'some reason',
            'details': 'Vestibulum sit amet dui commodo, lacinia ex eu, vestibulum odio. Suspendisse potenti.',
            'ap/bp': 'AP',
            'from': 't1',
            'to': 's1'
        },
    ...
    const tableRows = sortedData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => (
      <TableRow key={row.id} sx={{ backgroundColor: row['ap/bp'] === 'AP' ? 'green': 'red' }}>
        {columns.map((column) => (
          <TableCell key={column.id} sx={{ color: 'white'}}>{row[column.id]}</TableCell>
        ))}
      </TableRow>
    ));
